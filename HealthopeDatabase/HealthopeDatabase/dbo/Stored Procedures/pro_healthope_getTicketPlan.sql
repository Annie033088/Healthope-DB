CREATE PROCEDURE pro_healthope_getTicketPlan
	@status BIT,
...
BEGIN
	...
	SELECT f_ticketPlanId, f_price, f_status, f_updateTime
	INTO #tempTicketPlanTable
    FROM t_tickctPlan WITH(NOLOCK)
    WHERE  (@status IS NULL OR f_status = @status)

	SELECT @totalRecord = COUNT(*) FROM #tempTicketPlanTable WITH(NOLOCK); 
	SET @totalPage = CEILING(@totalRecord * 1.0 / @recordPerPage); --總頁數

	SELECT f_ticketPlanId, f_price, f_status, f_updateTime
    FROM #tempTicketPlanTable WITH(NOLOCK)
    ORDER BY
       CASE WHEN @sortOption IS NULL AND  @sortOrder = 'ascending' THEN CAST(f_ticketPlanId AS sql_variant) END ASC,
	   CASE	WHEN @sortOption IS NULL AND  @sortOrder = 'descending' THEN CAST(f_ticketPlanId AS sql_variant) END DESC,
       ...
       CAST(f_ticketPlanId AS SQL_VARIANT)  ASC --預設排序欄位
