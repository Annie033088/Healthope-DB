CREATE PROCEDURE [dbo].[pro_healthope_editOrderStatusAndPayByCash]
	@orderId INT, 
	@coachId INT,
...
BEGIN
	-- errorCode 定義
	DECLARE @success INT = 1, @modifiedFailed INT =11, @hasBeenModified INT = 14, @memberBaned INT = 26, @phoneNotVerify INT = 27, 
		@planNotAvailable INT = 28, @trackNotSet INT = 29, @coachBaned INT = 30
	-- 1.前置條件檢查
	-- 訂單資料
	DECLARE @memberId INT, @planId INT, @planType TINYINT, @planName NVARCHAR(20), @amount INT, @newUpdateTime DATETIME2(3)
	...
	BEGIN TRY
		...
		BEGIN
			...
			-- 同時檢查教練是否被禁用
			IF (SELECT f_status FROM t_coach WHERE f_coachId = @coachId) != 1
			BEGIN
		...
		-- 訂單狀態
		DECLARE @statusPending TINYINT = 1, @statusPaid TINYINT = 2
