CREATE PROCEDURE [dbo].[pro_healthope_editMembershipPlan]
 ...
 BEGIN
	...
	IF @updateTime != (SELECT f_updateTime FROM t_membershipPlan WITH(NOLOCK) WHERE f_membershipPlanId = @membershipPlanId)
		SET @errorCode = @hasBeenModified
	ELSE
	BEGIN
		UPDATE t_membershipPlan WITH(ROWLOCK)
		SET
		...
		OUTPUT 
		    CASE 
				...
				ELSE NULL
			END
		WHERE f_membershipPlanId = @membershipPlanId
