...
WHERE OBJECT_NAME(df.parent_object_id) = 't_admin'
  AND c.name = 'f_updateTime';

IF @constraintName IS NOT NULL
BEGIN
    SET @sql = N'ALTER TABLE t_admin DROP CONSTRAINT ' + QUOTENAME(@constraintName);
...
ALTER TABLE t_admin
ADD CONSTRAINT DF_t_admin_f_updateTime DEFAULT SYSUTCDATETIME() FOR f_updateTime;
