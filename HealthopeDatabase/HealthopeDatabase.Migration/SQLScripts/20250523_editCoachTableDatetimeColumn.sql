...
WHERE OBJECT_NAME(df.parent_object_id) = 't_coach'
...
BEGIN
    SET @sql = N'ALTER TABLE t_coach DROP CONSTRAINT ' + QUOTENAME(@constraintName1)
    EXEC sp_executesql @sql
END;

IF @constraintName2 IS NOT NULL
BEGIN
    SET @sql = N'ALTER TABLE t_coach DROP CONSTRAINT ' + QUOTENAME(@constraintName2)
    EXEC sp_executesql @sql
END;
...
ALTER TABLE t_coach
ADD CONSTRAINT DF_t_coach_f_createTime DEFAULT SYSUTCDATETIME() FOR f_createTime

ALTER TABLE t_coach
ADD CONSTRAINT DF_t_coach_f_updateTime DEFAULT SYSUTCDATETIME() FOR f_updateTime
